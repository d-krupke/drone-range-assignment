...
class PositionVars:
    ...
    def get_positions(self) -> typing.Dict[Drone, Point]:
        return {
            d: Point(self.x(d, True), self.y(d, True))
            for d in self.instance.drones
        }
    ...
    def __init__(self, model: gp.Model, instance: Instance) -> None:
        ...
        if len(instance.terminals) > 1:
            self.model.addConstr(
                sum(self._vars[(v, w)] for v in instance.terminals for w in instance.items() if v!=w) >= len(instance.terminals)
            )
    ...
    def lazily_enforce_strongly_connected(self) -> int:
        ...
        for t in self.instance.terminals:
            ...
            if any(t_ not in reachable_comp for t_ in self.instance.terminals):
                ...
                self.model.cbLazy(
                    sum(self._vars[(v, w)] for v in reachable_comp for w in self.instance.items() if w not in reachable_comp)>=1 
                )
                add_constraints += 1
...
class PowerVars:
    def __init__(self, sqdist: SquaredDistVars, edge_vars: EdgeVars) -> None:
        ...
        self._vars: typing.Dict[Agent, typing.Any] = {
            n: self.model.addVar(lb=0.0, ub=max_power, vtype=GRB.CONTINUOUS, name=f"r_{n}")
        ...
        for (a, b), var in edge_vars.items():
            if isinstance(a, Terminal) and isinstance(b, Terminal):
                ...
                self.model.addConstr(
                    self._vars[a] >=  var * (a.distance(b)**2)
                )
            else:
                # disable constraint if edge is not used
                self.model.addConstr(
                    self._vars[a] >= sqdist.ub(a, b) - (1-edge_vars.used(a, b)) * max_power
                )
...
class Solver:
    ...
    def solve(self, timelimit: float = 60.0) -> typing.Optional[Solution]:
        ...
        if self.model.numSolutions > 0:
            solution = Solution(self.instance, self.power.get_power_assignments(), self.positions.get_positions(), lb)
            return solution
        return None
